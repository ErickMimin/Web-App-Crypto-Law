...
def descifrarCaso( request ):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render( request, "errorInicio.html" )
...
def subirCambios( request, nombreAbogado, nombreDoc ):
    if request.method == "POST":
        ...
        if nombreArchivo == nombreDoc:
            ...
            firmaDigital = generarFirmaDigital( clavePrivadaRSA, contentModificado )
            resultVerificacion = verificarFirma( nameAbogado, contentModificado, firmaDigital )
            ...
            if resultVerificacion is False:
                return render( request, "sesionAbogado.html", {"user": nameAbogado, "idTelegram": idTelegramAbogado, 
                        ...
                        "alerta": "E Error: La firma digital no es valida, intenta nuevamente " } )
...
def descifrarDoc( request, nombreAbogado, numeroK, documento ):
    if request.method == "POST":
        ...
        keyAES = juntarLlaves( numeroAES, clavePrivadasAES, clavePrivadasRSA )
...
def modifDocCifrado( request, nombreAbogado, numeroK, documento ):
    if request.method == "POST":
        ...
        if nombreArchivo[2:] == documento:
            ...
            firmaDigital = generarFirmaDigital( clavePrivadasRSA[0], contentModificado )
            resultVerificacion = verificarFirma( nameAbogado, contentModificado, firmaDigital )
